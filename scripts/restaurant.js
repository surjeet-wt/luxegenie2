...
// Define the restaurant image sequence paths
const startFrame = 1;
const endFrame = 87;
const imagePaths = [];
for (let i = startFrame; i <= endFrame; i++) {
    if (i === 75 || i === 84) continue; // Skip missing frames
    imagePaths.push(`assets/sequences/restaurant/frame_${String(i).padStart(3, '0')}.jpg`);
}
...
async function startPreloader() {
    // 1. Immediately load the very first frame
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const tl = gsap.timeline({
        scrollTrigger: {
            trigger: "#restaurant-scroll-container",
            start: "top top",
            end: "+=380vh",
            scrub: true,
            pin: true,
    ...
    // Step 2: Scrub the image sequence frames
    tl.to(sequenceObj, {
        frame: totalFrames - 1,
        ease: "none",
        duration: 0.95,
    ...
    // Staggered pull-up animations for the features section
    ...
    // -------------------------------------------------------------
    // Restaurant Spaces Carousel Logic
    // -------------------------------------------------------------
